...
```{r}
plot(is_canceled~lead_time, data=hotel_data_numeric, col="light blue")
```
...
test <- hotel_data_numeric[index,]       
training <-hotel_data_numeric[-index,]  

train_model <- glm(is_canceled~., family = "binomial", data=training)
...
test$probability <- predict(train_model, test, type="response")
test$prediction <- ifelse(test$probability>0.5, "1" ,"0") 

mean(test$is_canceled == test$prediction)
...
Lets have a look at ROC and AUC parameters
...
auc(ROC)
```
AUC - ROC curve is a performance measurement for classification problem at various thresholds settings. ROC is a probability curve and AUC represents degree or measure of separability. It tells how much model is capable of distinguishing between classes. Higher the AUC, better the model is at predicting 0s as 0s and 1s as 1s. So basically our model coud predict around 70% right, which is quite good!
...
Looking a the numbers, it is clear why most people lose; infact, that is the whole business-scheme of lottery businesses. Make people believe
